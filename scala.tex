...
\begin{enumerate}
  ...
  \item Starting with the rightmost parent type and working left, compute the 
  linearization of each type, appending its linearization to the cumulative 
  linearization. (Ignore ScalaObject, AnyRef, and Any for now.)
  \item Working from left to right, remove any type if it appears again to the 
...
\begin{itemize}
  ...
  \lstinputlisting{examples/packages.scala}
...
\subsection{Symbols}
\begin{itemize}
  \item sind genauso wie in Ruby\footnote{:symbol}, also fixe Werte für 
  ein bestimmtes Symbol
  
  \lstinputlisting{examples/symbols.scala}  
\end{itemize}
\pagebreak
