...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item \texttt{foldLeft}\footnote{kann man gut verwenden, wenn man die Werte einer Sequenz aufsummieren will} nimmt einen einzelnen Parameter als
...
\begin{itemize}
  ...
  \item \itbf{pure object-oriented language} in the sense that every value is an
object and its mixin-based composition mechanism provides a clean alternative to the multiple-
inheritance models of other languages
  \item \itbf{funktionale Sprache}: By supporting higher-order functions, currying, function
nesting, pattern matching, and anonymous functions
  \item Scala wird entwede in Java Bytecode \oder \textit{.net assemblies}
  kompiliert. \textit{The integration with these
platforms is very tight. For example Java classes can be used from Scala code and vice versa. This
allows Scala programs to reuse the existing Java libraries and keeps the price for switching from
Java to Scala low.
}
  ...
  \item Klassen können von nur einer Klasse erben, aber es können von eine
  bis zu mehrere Klassen \textit{mixin} gemacht werden.
  ...
  \item Scala wurde mit dem Ziel designt, für alle Java-Sachen verträglich 
  zu sein. Deswegen läufts auch der JVM \und arbeitet reibungslos mit
  mit existierenden Javacode zusammen
  ...
  dis-tributed, component-based applications that support concurrency and
  distribution}
  \item immutable \texttt{val} müssen initialisiert werden!
  \item Scala unterstützt auch Symbole aus Ruby
  \item \texttt{null} gibts auch in Scala, soll man aber vermeiden, es ist
  besser $\Rightarrow$ besser \texttt{Option} verwenden, aus dem man die
  Null Information herausziehen kann
  \item statt Generics sagt man in Scala zu dynamischen Datentypen von
  Funktionen \itbf{parameterized types}
  \item \itbf{abstrakte Typen}:
  
  \lstinputlisting[label=src:abstract_types.scala]{beispiele/abstract_types.scala}
  
  die Klasse \texttt{BulkReader} hat drei abstrakte Members
  
  \item wenn man \textbf{final} vor Klassen \oder traits schreibt, dann verhindert man, dass davon Klassen abgeleitet werden können
...
interpreted scripts to large, sophisticated applications.


\ulbf{statically typed}: der Typ einer Variable ist für die gesamte Lebenszeit der Variable
  fest


\ulbf{Mixed Paradigm - OOP}
...
\begin{itemize}
  ...
  \item Funktionen sind \itbf{first class}, d.h. sie können an Variablen, an
  andere Funktionen usw. ähnlich wie Werte übergeben werden
  \item Scala bietet \itbf{closures} an, d.h. 
...
\begin{itemize}
  ...
  \lstinputlisting[label=src:scala_mit_javaklassen.scala]
